 ...
 *v1_2 also checks for suspended users and will not send emails to them. Updated to work with multiple REDCap_log_event tables. Antony Colles, Norwich Clinical Trials Unit, 2021
**/
...
$page = RCView::div(array('class'=>'autonotify_config'),
    RCView::h3(array('style'=>'color:#af8033;'),'AutoNotify').
	RCView::h5(array('style'=>'color:#e7a944;'),'Roles & DAGs 17/04/18 *** Record Link 19/04/18').
    $last_modified.
    $section.
    RCView::div(array(),
...
<script type="text/javascript">
    ...
    function save() {
        ...
        $('div.trigger', '#triggers_config').each (function (index1, value1) {
			...
			// Record link checkbox
			
			var linkAtt = $("input[name^=recordlink]", $(this));
			
			if (linkAtt.attr('checked') == "checked"){
				triggers[i]['recordlink'] = 1;
			} else {
				triggers[i]['recordlink'] = 0;
			}
	...
	// ARC 19/04/18 When you check/uncheck checkboxes, it doesn't really do anything useful
	function linkCheckBtn(pageEl){
		
		// If the input has been ticked, add the attribute 'checked' (removing 'unchecked')
		if ($(pageEl).is(':checked')) {
			if ($(pageEl).not('[checked]')) {
				$(pageEl).removeAttr("unchecked");
				$(pageEl).attr("checked", "checked");
			}
		} else {
			// Otherwise, do the opposite
			if ($(pageEl).not('[unchecked]')) {				
				$(pageEl).removeAttr("checked");
				$(pageEl).attr("unchecked", "checked");
			}
		}
	}
